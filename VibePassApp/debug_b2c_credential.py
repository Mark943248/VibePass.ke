...
def test_credential_generation():
    ...
    try:
        ...
        print(f" Initiator Password: {'*' * len(initiator_password) if initiator_password else 'NOT SET'}")
        ...
        if len(security_credential) > 100 and security_credential.replace('=', '').replace('/', '').replace('+', '').isalnum():
...
def test_mpesa_credentials():
    ...
    print(f" Consumer Secret: {consumer_secret[:10]}...{consumer_secret[-10:] if consumer_secret else 'NOT SET'}")
...
def main():
    ...
    print("║" + f"  Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}".center(58) + "║")
    print("║" + " " * 58 + "║")
    print("╚" + "=" * 58 + "╝")
