...
def HomePage(request):
    ...
    context = {"recent_events": recent_events, "today": date_today}
...
def add_scanner(request):
    ...
    organisers_scanners = EventScanner.objects.filter(
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if event.Event_date < today:
                logger.warning(
                    f"Attempt to add scanner to past event '{event.Event_title}' by {request.user.username}"
                )
                messages.error(
                    request,
                    f"Cannot add scanner. The event '{event.Event_title}' has already ended.",
                )
                return redirect("add_scanner")

            if not event.Event_is_active:
                logger.warning(
                    f"Attempt to add scanner to inactive event '{event.Event_title}' by {request.user.username}"
                )
                messages.error(
                    request,
                    f"Cannot add scanner. The event '{event.Event_title}' is currently inactive.",
                )
                return redirect("add_scanner")
            ...
            if EventScanner.objects.filter(event=event, user=scanner_user).exists():
                messages.error(
                    request,
                    f"{scanner_user.username} is already a scanner for {event.Event_title}.",
                )
                return redirect("add_scanner")
            else:
                # Include added_by=request.user to track accountability!
                EventScanner.objects.create(
                    event=event, user=scanner_user, added_by=request.user
                )
                logger.info(
                    f"User {scanner_user.id} added as scanner for event {event.id} by {request.user.id}"
                )
                messages.success(
                    request,
                    f"{scanner_user.username} has been added as a scanner for {event.Event_title}.",
                )
                return redirect("add_scanner")

        except Event.DoesNotExist:
            logger.error(
                f"Event with slug {event_slug} not found or unauthorized access attempt by {request.user.username}"
            )
            messages.error(
                request,
                "Event not found or you do not have permission to add scanners for this event.",
            )
            return redirect("add_scanner")
            ...
            return redirect(
                "add_scanner"
            )  # Added redirect here to catch user lookup failures cleanly
...
def remove_scanner(request, scanner_id):
    ...
    """
    try:
        scanner = EventScanner.objects.select_related("user", "event").get(
            scanner_id=scanner_id, added_by=request.user
        )
        ...
        logger.info(
            f"Scanner {username} removed from event {event_title} by {request.user.username}"
        )
        messages.success(
            request, f"{username} has been removed as a scanner for {event_title}."
        )

    except EventScanner.DoesNotExist:
        logger.error(
            f"Attempt to remove non-existent or unauthorized scanner with ID {scanner_id} by {request.user.username}"
        )
        messages.error(
            request,
            "Scanner not found or you do not have permission to remove this scanner.",
        )
