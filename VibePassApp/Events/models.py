...
class Event(models.Model):
    ...
    def min_ticket_price(self):
        price = self.ticket_types.filter(is_active=True).aggregate(
            min_price=Min("price")
        )["min_price"]
        if price is None:
...
class TicketType(models.Model):
    ...
    )  # Additional description for the ticket type
    price = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    group_size = models.PositiveIntegerField(
        default=1
    )  # Number of tickets that can be purchased together
    capacity = (
    ...
    class Meta:
        ...
        unique_together = (
            "event",
            "name",
        )  # Prevent duplicate ticket types for the same event
...
class EventScanner(models.Model):
    ...
    user = models.ForeignKey(
        "Users.User", on_delete=models.CASCADE, related_name="scanned_events"
    )
    ...
    class Meta:
        unique_together = (
            "event",
            "user",
        )  # Prevent duplicate scanner entries for the same event
