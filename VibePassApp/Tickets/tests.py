...
class TicketViewsTest(TestCase):
    ...
    def test_book_free_ticket_not_free(self):
        ...
        self.assertRedirects(
            response, reverse("event_details", args=[self.paid_event.slug])
        )
    ...
    def test_book_free_ticket_success(self):
        ...
        self.assertTrue(
            Ticket.objects.filter(event=self.free_event, user=self.user).exists()
        )
    ...
    def test_create_ticket_function(self):
        ...
        payment = Payment.objects.create(
            ...
            checkout_data_snapshot={
                "items": [{"id": self.paid_ticket_type.id, "quantity": 1}]
            },
        )
